...
If a Feather script or function reaches the end **without calling `success`**, it will automatically be marked as a fail. This enforces explicit intent and ensures that scripts never exit silently.
...
- If a Feather script does not call `success` anywhere, it will automatically be marked as a fail when it reaches the end.
